...
const handleSubmit = () => {
    ...
    const allData = {
        guardians,
        selectedFiles: selectedFiles.map(file => file ? { name: file.name, type: file.type, size: file.size } : null), // Store file metadata
        selectedDeathCertificateFiles: selectedDeathCertificateFiles.map(file => file ? { name: file.name, type: file.type, size: file.size } : null), // Store file metadata
        selectedDocumentTypes,
        deathCertificateDocumentTypes,
    };
            ...
            {(!isOtherGuardian && guardian.isDeceased === false) || isOtherGuardian ? (
                <>
                    ...
                    <div className="flex flex-col sm:flex-row items-start gap-4 mb-4">
                        ...
                        <div className="w-[400px]"> {/* Genişlik burada ayarlanıyor */}
                            <Select
